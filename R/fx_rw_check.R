...
rw_check <- function(speciesFile, plotLocations, counties){
  ...
  species_richness_rw <- read.csv(speciesFile)
  ...
  species_richness_rw_spatial <- read.csv(plotLocations)
  ...
  # Transform county spatial dataspeciesFile to match the site coordinates 
